...
class vm_menu{
	 ...
	 * 
	 */
	public function vm_set_admin_view(){
		wp_register_script('plugin-menu',VM_REORDER_PATH.'js/menu.js');
		wp_register_style('vm-plugin-style',VM_REORDER_PATH.'css/style.css');	
		wp_register_style('w3-css',VM_REORDER_PATH.'css/w3.css');
	}
	/**
	 *	Add Setting Page
	 * 
	 */
	function vm_add_setting_page()
	{
		if(is_super_admin())
		{
			add_options_page( 'Reorder Admin Menus', 'Reorder Admin Menus', 'manage_options', 'reorder_menu_page', array($this,'vm_add_menu_options_test'));		
		}
	}
	/**
	 * Set option values which will use
	 * in menu manipulation.
	 * 
	 */
	public function vm_plugin_override() {
		global $menu;
		update_option('mr_original_position',$menu);
		$custom_orginal_menu = array();
		$menu_with_array_value = array();
	 ...
	 * 
	 */
	public function vm_custom_menu_order($menu_ord){
	...
	/**
	 *	Set Menu order for all the use 
	 * 	@menu
	 */
	public function vm_set_menuorder()
	{
	...
	/**
	 * 	Set Menu order for particular user role
	 * 	@user_role
	 */
	public function vm_set_menuorder_by_user_role()
	{
		if(isset($_REQUEST['_nonce']))
		{
			$nonce = $_REQUEST['_nonce'];
	...
	/**
	 * Get newly set menu by user role
	 * Called on onChange event of user role dropdown
	 */
	public function vm_get_menuorder_by_user()
	{
		if(isset($_REQUEST['_nonce']))
		{
			$nonce = $_REQUEST['_nonce'];
	 ...
	 * Make it as it was
	 */
	public function vm_set_to_default()
	{
		global $wp_roles;
	...
	/**
	 * Reset menu for selected user role
	 * @user_role
	 */
	public function vm_reset_for_userrole()
	{
		if(isset($_REQUEST['_nonce']))
		{
			$nonce = $_REQUEST['_nonce'];
	...
	/**
	 * Set Setting Page layout for vm_menu_reorder
	 * @page
	 */
	function vm_add_menu_options_test()
	{
		global $menu;
		wp_enqueue_script('jquery-ui-sortable');
		wp_enqueue_script('plugin-menu');
		wp_enqueue_style('w3-css');
		// wp_enqueue_style('j-ui-css');
		wp_enqueue_style('vm-plugin-style');
	?>
